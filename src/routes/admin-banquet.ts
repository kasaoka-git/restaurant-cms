        ...
        <div id="modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
            <div class="bg-white rounded-lg p-8 max-w-2xl w-full mx-4 my-8">
                ...
                <form id="item-form" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">コース画像</label>
                        <p class="text-xs text-gray-500 mb-2">推奨サイズ: 1200px × 800px | 形式: JPEG/PNG | <strong class="text-red-600">最大: 3MB</strong></p>
                        
                        <!-- Tab Navigation -->
                        <div class="flex space-x-2 mb-4 border-b">
                            <button type="button" onclick="switchBanquetTab('upload')" id="tab-upload"
                                    class="px-4 py-2 border-b-2 border-blue-600 text-blue-600 font-medium">
                                アップロード
                            </button>
                            <button type="button" onclick="switchBanquetTab('url')" id="tab-url"
