...
import type { Bindings, MenuImage } from '../types'
import { requireAuth } from '../utils/auth'
import { getAdminLayout } from '../utils/admin-layout'

const app = new Hono<{ Bindings: Bindings }>()

// Menu Images List Page
app.get('/', requireAuth, async (c) => {
  ...
  const content = `
        <div class="max-w-6xl">
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-700">
                            メニュー画像は最大50枚まで登録できます。ドラッグ&ドロップで表示順序を変更できます。
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6 bg-yellow-50 border-l-4 border-yellow-500 p-4">
                <p class="text-sm text-yellow-700">
                    ...
                    最大数（50枚）に達しています。新しく追加するには既存のものを削除してください。
                </p>
            </div>
            `}
            ...
            <div id="items-list" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-lg shadow-lg overflow-hidden cursor-move" data-id="${item.id}">
                    <div class="aspect-square relative group">
                        <img src="${item.image_url}" alt="メニュー ${index + 1}" class="w-full h-full object-cover">
                        <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition flex items-center justify-center">
                            <i class="fas fa-grip-vertical text-white text-2xl opacity-0 group-hover:opacity-100 transition"></i>
                        </div>
                    </div>
                    <div class="p-4">
        ...
        <div id="modal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
            <div class="bg-white rounded-lg p-8 max-w-2xl w-full mx-4 my-8">
                <div class="flex justify-between items-center mb-6">
                    <h2 id="modal-title" class="text-2xl font-bold">メニュー画像を追加</h2>
                ...
                <form id="item-form" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">メニュー画像 <span class="text-red-600">*</span></label>
                        <p class="text-xs text-gray-500 mb-2">推奨サイズ: 1200px × 1600px | 形式: JPEG/PNG | <strong class="text-red-600">最大: 1MB</strong></p>
                        
                        <!-- Tab Navigation -->
                        <div class="flex space-x-2 mb-4 border-b">
                            <button type="button" onclick="switchMenuTab('upload')" id="tab-upload"
                                    class="px-4 py-2 border-b-2 border-blue-600 text-blue-600 font-medium">
                                アップロード
                            </button>
                            <button type="button" onclick="switchMenuTab('url')" id="tab-url"
        ...
        <script>
          ...
          // Initialize uploader with 1MB limit
          ...
          if (itemsList && itemsList.children.length > 0) {
            new Sortable(itemsList, {
              ...
              onEnd: async function(evt) {
                ...
                try {
                  await axios.post('/admin/api/menu/reorder', { updates });
                  location.reload();
                } catch (error) {
          ...
          function openAddModal() {
            isEditMode = false;
            document.getElementById('modal-title').textContent = 'メニュー画像を追加';
          ...
          function editItem(item) {
            isEditMode = true;
            document.getElementById('modal-title').textContent = 'メニュー画像を編集';
          ...
          async function deleteItem(id) {
            if (!confirm('このメニュー画像を削除しますか？')) return;
            
            try {
              await axios.delete('/admin/api/menu/' + id);
              location.reload();
            } catch (error) {
          ...
          document.getElementById('item-form').addEventListener('submit', async (e) => {
            ...
            try {
              if (isEditMode) {
                await axios.put('/admin/api/menu/' + data.id, data);
              } else {
                await axios.post('/admin/api/menu', data);
              }
  ...
  return c.html(getAdminLayout(content, 'menu', 'メニュー画像管理'));
})
...
app.post('/', requireAuth, async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    'SELECT COUNT(*) as count FROM menu_images'
  ).all();
  
  if (results[0].count >= 50) {
    return c.json({ success: false, message: '最大50枚までしか登録できません' }, 400);
  }
  
  const maxOrder = await c.env.DB.prepare(
    'SELECT MAX(display_order) as max_order FROM menu_images'
  ).first();
  
  const nextOrder = (maxOrder?.max_order || 0) + 1;
  
  await c.env.DB.prepare(`
    INSERT INTO menu_images (image_url, display_order)
    VALUES (?, ?)
  `).bind(data.image_url, nextOrder).run();
...
app.delete('/:id', requireAuth, async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    'SELECT id FROM menu_images ORDER BY display_order ASC'
  ).all();
  
  for (let i = 0; i < results.length; i++) {
    await c.env.DB.prepare(
      'UPDATE menu_images SET display_order = ? WHERE id = ?'
