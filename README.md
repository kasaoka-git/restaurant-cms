...
- ✅ メインイメージ管理（追加・編集・削除・並び替え）
- ✅ こだわり情報管理（追加・編集・削除・並び替え・表示切替）
- ✅ ご挨拶編集（タイトル・テキスト・画像）
- ✅ メニュー画像管理（最大50枚、並び替え）
- ✅ 宴会コース管理（追加・編集・削除・並び替え・表示切替）
- ✅ 新着情報管理（追加・編集・削除）
- ✅ ギャラリー管理（追加・編集・削除・並び替え）
- ✅ FAQ管理（追加・編集・削除・並び替え）
...
- ✅ リアルタイムプレビュー機能
- ✅ アップロード進捗表示
- ✅ ファイル形式検証（JPEG, PNG, GIF, WebP, MP4, WebM, MOV）
- ✅ 全管理画面への統合（メインイメージ、こだわり情報、宴会コース、ギャラリー）
...
すべての管理画面機能とアップロード機能が実装されました：

### 管理画面機能
1. **メインイメージ管理** - 最大3つのスライダー画像/動画（アップロード対応）
2. **こだわり情報管理** - 1〜6個のこだわりポイント（アップロード対応）
3. **ご挨拶管理** - タイトル・本文・画像の編集
4. **メニュー画像管理** - 最大50枚のメニュー画像
5. **宴会コース管理** - コース情報の追加・編集・削除（アップロード対応）
6. **新着情報管理** - お知らせの投稿・編集
7. **ギャラリー管理** - 写真ギャラリーの管理（アップロード対応）
8. **FAQ管理** - よくある質問の編集
